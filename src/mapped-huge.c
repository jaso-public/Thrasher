...
*/
static uint8_t code[] = {0x48, 0xff, 0xcf, 
                    0x75, 0x01, 
                    0xc3, 
                    0x48, 0x01, 0xd8,
                    0x48, 0x39, 0xc1,
...
int main(int argc, char** argv) {
   ...
   int c;
    while ((c = getopt(argc, argv, "v:f:n:p:s:i:a:h:")) != -1) {
        switch (c) {
            case 'f':
    ...
    if (ftruncate(fd, page_size) == -1) {
     ...
     for(int i=0; i<num_pages; i++) {
        // Memory map the file
        void *addr = mmap(desired_addr, page_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED, fd, 0);
        if (addr == MAP_FAILED) {
            perror("mmap");
            close(fd);
            exit(EXIT_FAILURE);
